...
import com.kadioglumf.email.payload.request.search.ConditionRequest;
import com.kadioglumf.email.payload.request.search.FilterRequest;
import com.kadioglumf.email.payload.request.search.Operator;
...
public enum FilterType {
    ...
    EQUAL {
        public <T> Predicate build(Root<T> root, CriteriaBuilder cb, FilterRequest request,
                                   ...
                                   SearchExpressionType searchExpressionType, SearchRoleType searchRoleType) {

            Object value = request.getFieldType().parse(validateValueAndReturn(condition.getValue()).toString());
            Expression<?> key = searchExpressionType.getExpression(root, request.getKey(), searchRoleType);
    ...
    CONTAINS {
        public <T> Predicate build(Root<T> root, CriteriaBuilder cb, FilterRequest request,
            ...
            if (FieldType.STRING.equals(request.getFieldType())) {
                ...
                Expression<String> key = (Expression<String>) searchExpressionType.getExpression(root, request.getKey(), searchRoleType);
                if (predicate == null) {
        ...
        public <T> Predicate build(Root<T> root, CriteriaBuilder cb, FilterRequest request,
            ...
            if (FieldType.STRING.equals(request.getFieldType())) {
                ...
                if (value instanceof LocalDateTime) {
                    ...
                    Expression<LocalDateTime> key = (Expression<LocalDateTime>) searchExpressionType.getExpression(root, request.getKey(), searchRoleType);
                    ...
                    Expression<LocalDate> key = (Expression<LocalDate>) searchExpressionType.getExpression(root, request.getKey(), searchRoleType);
                ...
                Expression<Number> key = (Expression<Number>) searchExpressionType.getExpression(root, request.getKey(), searchRoleType);
                if (predicate == null) {
    ...
    public Object validateValueAndReturn(Object obj) {
        if (obj == null) {
            throw new EmailServiceException(ErrorType.FILTER_PARAMETER_CANNOT_NULL_ERROR);
        }
        return obj;
    }
